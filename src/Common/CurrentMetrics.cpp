...
#define APPLY_FOR_BUILTIN_METRICS(M) \
    ...
    M(AllocatorCachedMemoryTracking, "Total amount of memory (bytes) held in malloc caches and arenas.") \
    M(AllocatorAllocatedMemoryTracking, "Total amount of memory (bytes) malloc'ed and not free'd.") \
